...
func TestDiffVarintCodec(t *testing.T) {
	...
	postingsMap := map[string]index.Postings{
		...
		`n="1"`:    matchPostings(t, idx, labels.MustNewMatcher(labels.MatchEqual, "n", "1"+benchutil.LabelLongSuffix)),
		...
		`n!="2"`:   matchPostings(t, idx, labels.MustNewMatcher(labels.MatchNotEqual, "n", "2"+benchutil.LabelLongSuffix)),
